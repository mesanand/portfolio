...
- **SGA SOG Board:** replaces the "Student Government Governance Board" entry (same role): Secretary, Sep 2024 to Feb 2025, "Reviewed constitutions and bylaws for 500+ organizations under the director of student involvement. Took meeting notes and organized information for several members of the board." Boston era, not Oakland.
- **Logos on the four current leadership cards** (overrides prompt 7's "no logos for organizations"): Claude Builders Club (claudeneu.com favicon), ACM (the chapter's own "ACM" gear-and-globe logo, supplied by Mehr), Rev (rev.school logo), NIBG (its LinkedIn logo). In `src/assets/logos/`, centered in the space right of each card's heading.
- **Slice of my life:** photos only, no titles or captions under them. Removed: NYC Convocation panel, Societies of Distinction, Claude Builders Club meeting. Added at the front: beach chair, jet ski, hat stack, dinner, fencing club (the NYC rooftop photo was added, then removed).
- **Home Projects section** shows only Chordly and Inquisiv (`featured: true`); /projects lists all. Chordly links its CS 4100 executive summary PDF (`public/projects/chordly-executive-summary.pdf`) as [ REPORT ].
...
- **NETWORK** (`/network`, nav after HIGHLIGHTS): the Tower, a separate Vercel project served at https://network.mehr-anand.com/network/*, proxied into this site by the first two rewrites in vercel.json. No client-side route; the nav item is a plain <a href="/network"> (full page load) in the header, phone menu, and footer.
- **SGA SOG Board** is Boston era.
- **No resume on the site** (Mehr): the RESUME header button, phone-menu item, and footer link are removed. Visitors use [ REQUEST MY RESUME ] (Home contact section and /work), a mailto to anand.me@northeastern.edu with the subject "Resume request". Supersedes 07 s1 and s7 item 1 (resume.pdf).
